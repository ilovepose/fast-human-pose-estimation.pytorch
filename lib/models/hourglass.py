...
class HourglassNet(nn.Module):
    ...
    def forward(self, x):
        out = []
        x = self.conv1(x)  # 经过stride为2的7*7卷积从256变成128
        x = self.bn1(x)
        x = self.relu(x) 

        x = self.layer1(x)  
        x = self.maxpool(x)  # 经过maxpool从128变成64
        x = self.layer2(x)  
        x = self.layer3(x)  

        for i in range(self.num_stacks):
            y = self.hg[i](x)  # 经过一个hourglass模块
            y = self.res[i](y)
            y = self.fc[i](y)
            score = self.score[i](y)  # 最后输出17个关键点热图
            out.append(score)
...
class HourglassNetPro(nn.Module):
    '''Hourglass model from Newell et al ECCV 2016'''
    def __init__(self, block, cfg, **kwargs):
        super(HourglassNetPro, self).__init__()
        # Parameters: num_feats=256, num_stacks=8, num_blocks=1, num_classes=16
        extra = cfg.MODEL.EXTRA
        num_feats = extra.NUM_FEATURES
        num_stacks = extra.NUM_STACKS
...
class HourglassNet33(nn.Module):
    '''Hourglass model from Newell et al ECCV 2016'''
    def __init__(self, block, cfg, **kwargs):
        super(HourglassNet33, self).__init__()
        # Parameters: num_feats=256, num_stacks=8, num_blocks=1, num_classes=16
        extra = cfg.MODEL.EXTRA
        num_feats = extra.NUM_FEATURES
        num_stacks = extra.NUM_STACKS
...
def get_pose_net(cfg, is_train, **kwargs):
    # TODO:
    if cfg.MODEL.NAME2 == 'hourglass':
        model = HourglassNet(Bottleneck, cfg, **kwargs)
    elif cfg.MODEL.NAME2 == 'hourglasspro':
        model = HourglassNetPro(Bottleneck, cfg, **kwargs)
    elif cfg.MODEL.NAME2 == 'hourglass33':
        model = HourglassNet33(Bottleneck, cfg, **kwargs)
    else:
